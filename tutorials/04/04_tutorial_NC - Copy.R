...
# Step 1: Assumptions #NCcategorical values ...
# Step 2: Hypotheses 
# Step 3: Test statistic #NCthe X value
# Step 4: P-value
# Step 5: Conclusion #NCwe can reject null hypothesis and movie and rating genres are not independent
...
chi_test$expected # f_e (expected frequencies under the assumption of H0, #NCthat if independent
...
# How can we interpret the standardized residuals? #NC comment - these are contigency tests. 
...
# Add to scatter plot #NC adding the correlation figure on the table 
text(1200, 7, sprintf("Correlation=%s", round(cor(df$income,df$edu),4)))

# Improve visualization and save #NC SAVE IN YOUR FOLDER DEV OFF
...
# Step 2: Hypotheses #that no association, correlation
# Step 3: Test statistic #
# Step 4: P-value
# Step 5: Conclusion #there is more evidence that there is a correlation between income and correlation, and it's positive. 
...
#NC COMMENT THE THREE STARS GIVE AN INDICATION OF INTERVAL
...
# Step 1: Assumptions #measurement scale is continuos  
# Step 2: Hypotheses #slope = 0
# Step 3: Test statistic # reject the null hypothesis, and there is strong evidence that there is a relationship between education and income 
...
# Plot predictions
plot(x=df$edu, y=df$income) # Scatter plot
points(df$edu, model$fitted.values, # Add another scatter plot on top #NC on top of an existing plot!!!
       col="green")
